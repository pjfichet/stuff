...
" also: .lds .ev
syn match nroffReqname /\([da]s\|lds\|ev\)/ contained nextgroup=nroffDefIdent skipwhite
...
" also .ch .em .so
syn match nroffReqName /\(d[ei]\|am\|ch\s\|em\|so\)/ contained nextgroup=nroffIdent skipwhite
...
if exists("b:nroff_is_troff")
	syn match nroffReqName /\(rnn\|aln\|lnr\)/ contained nextgroup=nroffIdent skipwhite
endif

" {{{2 Utmac
syn match nroffReqName /\(H[0-9*]\)/ contained nextgroup=utmacHead skipwhite
syn match utmacHead /.\+/ contained contains=nroffEscape
syn match nroffReqname /\(D[AIKST]\)/ contained nextgroup=utmacData skipwhite
syn match utmacData /.\+/ contained contains=nroffEscape
syn match nroffReqName /\(R[VIHLOPN]\)/ contained nextgroup=utmacRegi skipwhite
syn match utmacRegi /.\+/ contained contains=nroffEscape
syn match utmacIndx /{[^}]*}/
...
syn region nroffRefer start=/^\.\s*\[/ end=/^\.\s*\]/ contains=referKey
syn match referKey /^%/	contained nextgroup=referField
syn match referField /./ contained nextgroup=referString skipwhite
syn match referString /.\+/ contained contains=nroffEscape
...
if version >= 508 || !exists("did_nroff_syn_inits")
	...
	HiLink utmacHead Head
	HiLink utmacData Data
	HiLink utmacRegi Regi
	HiLink utmacIndx Indx

	HiLink referData nroffString
	HiLink referKey PreProc
	HiLink referField PreProc
	HiLink referString nroffString
